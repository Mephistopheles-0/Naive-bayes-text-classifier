...
function train_naive_bayes(documents, labels)
...
# Define a function to classify new documents using the trained model
function classify_naive_bayes(document, unique_labels, prior_probs, word_probs)
    ...
    for i in 1:length(unique_labels)
        ...
        for word in words
            if haskey(word_probs[label], word)
                label_probs[i] += log(word_probs[label][findfirst(unique(words), word)])
            end
        end
    end
